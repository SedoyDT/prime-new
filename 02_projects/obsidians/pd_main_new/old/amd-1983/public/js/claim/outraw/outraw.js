/**
 * Заявка "Отгрузка сырья"
 */
var claim = new (function()
{
    this.formId        = 'outraw-form';
    ...
    this.start = function()
    {
        this.ngReserveController = $('[ng-controller="ReserveController"]');
        ...
        $('.guarantee').datepicker({duration: 'fast', showAnim: 'fadeIn'});
        ...
        if (claim.isSubsidiaryProject && parseInt(claim.claimStatus) !== 0) {
            ...
            $('.editProduct').live('click', function()
            {
                ...
                // Все остальные товары
                    ...
                    claim.displayEditProductStart(this.lang, true);
                }
        ...
        if (window.Dispatcher) {
            new Dispatcher({
                ...
                updateCarListActionUrl: '/claim/outraw/get-dispatcher-drivers/'
            })
        }
        ...
        if (window.LeadId) {
            new LeadId({
                ...
                field: $('.js-lead-field'),
                text: $('.js-lead-text'),
                projectField: $('.js-lead-project-field'),
                url: $('.js-lead-url'),
    ...
    /**
     * Одинакрный клик по товару в гриде
     * @param element
     * @param productId
     */
    this.depotProductsGridOnClick = function (element, productId)
    {
        var clickedRow  = element.closest('tr');
        var siblingRows = clickedRow.siblings('tr');

        // Убрать выделение у соседних строк
        siblingRows.removeAttr('data-selected-for-reserve');

        // Обработать текущую строку
        if (parseInt(clickedRow.attr('data-selected-for-reserve'))) {
            clickedRow.removeAttr('data-selected-for-reserve');
        } else {
            clickedRow.attr({
                'data-selected-for-reserve': productId
            });
        }
    };
     ...
     * @param event
     */
    this.createOrUpdateReserve = function (event)
    {
        var productId     = parseInt($('tr[data-selected-for-reserve]').attr('data-selected-for-reserve')) || 0;
        var desiredAmount = parseFloat($(event.target).parent().find('input[type="text"]').val());
        var claimId       = parseInt(claim.getClaimOutId());

        if (!productId) {
            return alert('Не выбран товар!');
        }

        if (!desiredAmount || desiredAmount < 1) {
            return alert('Необходимо указать резервируемой количество!');
        }

        if (!claimId) {
            return alert('Не удалось определить ID заявки!');
        }

        claim.togglePageOverlay(true);

        $.ajax({
            url: '/claim/outraw/products-service',
            data: {
                claimId: claimId,
                command: 'createOrUpdateReserve',
                reserveParams: {
                    claimId: claimId,
            ...
            success: function (response)
            {
                if (response.success === false) {
                    ...
                    if (response.hasOwnProperty('products')) {
                        claim._displayEditproductEnd(response.products);
                    }
                    // Обновить список резервов
                    if (response.hasOwnProperty('products')) {
                        angular.element(claim.ngReserveController).scope().updateReserveTree(response.reserveTree, true);
                    }

                    alert('Резерв сохранен!');
                }
            },
            complete: function ()
            {
                claim.togglePageOverlay(false)
            }
        });
    };
     ...
     * @param action
     */
    this.togglePageOverlay = function (action)
    {
        $('#page-overlay').toggle(action);
    };
    ...
    /**
     * Округляет число с точностью до 5 знаков
     * @param number
     * @returns {number}
     */
    ...
    /**
     * Удаляет все дочерние элементы из заданного контейнера
     * @param obj
     */
     ...
     * @type {{parent: *, managerList: Array, filter: Window.manager.filter, change: Window.manager.change, value: Window.manager.value}}
     */
    this.manager =
    {
        parent: this,
    ...
    /**
     * Фильтр для клиентов
     * @type {{parent: *, clientList: Array, filter: Window.client.filter, change: Window.client.change, value: Window.client.value}}
     */
    this.client =
    {
        parent: this,
        ...
        filter: function(obj)
        {
            ...
            for (var i = 1; i < this.clientList.length; i++) {
                ...
                if (this.clientList[i].text.toLowerCase().indexOf(val) === -1 && parseInt(this.parent.node(obj.lang).lastValue) !== parseInt(this.clientList[i].id)) {
                    continue;
                }
        ...
        change: function(obj, not)
        {
            if (parseInt(obj.lastValue) && obj.value !== obj.lastValue && claim.hasActiveReserves()) {

                if (confirm("В заявке есть зарезервированный товар\nИзменение клиента удалит блокировки и обновит заявку!")) {

                    claim.togglePageOverlay(true);

                    // Разблокировать товар и обнулить резерв
                    $.ajax({
                        url: '/claim/outraw/products-service/',
                        cache: false,
                        type: 'POST',
                        ...
                        async: false,
                        data: {
                            claimId: claim.getClaimOutId(),
                            command: 'changeClient',
                            clientId: parseInt(obj.value)
                        },
                        success: function (response)
                        {
            ...
            claim.node('claim-client-work-time').innerHTML = '<span style="font-size: 10pt; font-weight: bold;">Время работы: </span>' + (obj.selectedIndex === -1 ? 'не определено' : obj.options[obj.selectedIndex].getAttribute('lang'));
        },
        value:function() {
    ...
    /**
     * Возвращает флаг о наличии активных резервов
     * @returns {number}
     */
    this.hasActiveReserves = function()
    {
        var result = 0;

        if (claim.reserveTree.hasOwnProperty('reserves')) {
            $.each(claim.reserveTree.reserves, function(index, reserve)
            {
                if (parseFloat(reserve.desiredAmount) > 0) {
    ...
    /**
     * Дата заявки
     * @type {{change: Window.claimDate.change}}
     */
    this.claimDate =
    {
        change: function(obj)
    ...
    /**
     * Счета
     * @type {{invoicesElId: string, listInvoices: Array, usedInvoices: {}, totalSum: number, withoutDocs: boolean, creditLimit: number, currentSum: number, block: boolean, deferment: null, default_option: string, maxCountSelects: number, currentCountSelects: number, show: Window.invoices.show, hide: Window.invoices.hide, getInvoices: Window.invoices.getInvoices, reset: Window.invoices.reset, change: Window.invoices.change, validate: Window.invoices.validate, addSelect: Window.invoices.addSelect, isBlock: Window.invoices.isBlock, blockSendInput: Window.invoices.blockSendInput, load: Window.invoices.load}}
     */
    this.invoices =
    {
        ...
        {
            $('#outraw-form input[value="отправить"]').prop('disabled', this.isBlock());
        },
        load: function()
        {
    ...
    /**
     * Машина
     * @type {{parent: *, listCars: Array, tid: string, changeCar: Window.car.changeCar, changeNumber: Window.car.changeNumber}}
     */
    this.car =
    {
        parent: this,
        ...
        changeNumber: function(obj, idChangedCar)
        {
            ...
            if (idChangedCar) {
                for (var key in claim.dataContainer.get('carList')) {
                    ...
                    this.parent.element('claim[driverName]').value = claim.dataContainer.get('carList')[key].name;
                    this.parent.element('claim[driverPhone]').value = claim.dataContainer.get('carList')[key].phone;
     ...
     * @type {{parent: *, change: Window.kickback.change, oninput: Window.kickback.oninput}}
     */
    this.kickback = {
        parent: this,
        change: function(obj)
     ...
     * @param obj
     */
    this.changeSection = function(obj)
    {
        obj.value = obj.value.replace(/[,]/g, '.');
        ...
        var ctrl = this.uncompresData(obj.lang);
        var maxVal = ctrl.maxamount - ctrl.confirmAmount;

        if ( ctrl.curblocks - ctrl.confirmBoxes - ctrl.blockboxes > 0 || ctrl.curblocks < ctrl.maxboxes) {
            maxVal -= 1;
        }

        if (obj.value > maxVal || ctrl.blockboxes + ctrl.confirmBoxes == ctrl.maxboxes ) {
            obj.value = this.round(maxVal);
        }

        var number = obj.parentNode.parentNode.getAttribute('pnumber');
        var reg = new RegExp('^products\\[' + number + '\\]\\[sections\\]\\[\\d+\\]\\[amount\\]$');
        var total = 0;
        var elements = this.node(this.formId).elements;

        for (var i = 0; i < elements.length; i++) {
            if (reg.test(elements[i].name)) {
                total += this.toFloat(elements[i].value);
            }
        }

        this.node(this.formId)['products[' + number + '][amount]'].value = this.round(total);
        this.changePrice(this.node(this.formId)['products[' + number + '][price]']);
    };
    ...
    this.applyProductFilter = function(filter)
    {
        ...
        $.ajax({
            url      : '/claim/outraw/getproductfilterdata',
            cache    : false,
            type     : 'POST',
            dataType : 'json',
            data     : data,
    ...
    /**
     * Выбор товара для добавление
     * @param clientProducts
     * @param productTypes
     */
    this.showSelectProducts = function(clientProducts, productTypes)
    {
        if (clientProducts) {
            if (!parseInt($('#client_select').val())) {
        ...
        this.overley.clientProduct = clientProducts;
        this.overley.onAfterCreate = function() {
            ...
            this.filter = new Filter({
                urlGetFullData:linkPrefix + '/claim/outraw/getoutfilterconfig',
                urlGetMaskData:linkPrefix + '/claim/outraw/getoutmaskfilter',
                filterType:'excel',
                conteinerId:'filter',
            ...
            this.grid = new Grid({
                conteinerId:'grid',
                urlGetData:linkPrefix + '/claim/outraw/getoutresult',
            ...
            if (this.clientProduct) {
                this.filter.addPostData = {clientid : parseInt($('#client_select').val())};
            }

            this.grid.head = {
                id:      {caption: 'ID',           width:43,   sort:1, aliace:function(v, cnt, rowData){ return '<span style="text-decoration:underline; cursor:pointer;">' + v + '</span>';}},
                title:   {caption: configTitle,    width:115,  sort:1, aliace:claim.aliace, align:'left'},
                ...
                baseprice:    {caption:'БЦ', width:75, sort:1, aliace:function(v, cnt, rowData){ return (parseFloat(v)||'');}},
                baseprice_in: {caption:'Закупочная БЦ', width:75, sort:1, aliace:function(v, cnt, rowData){ return (parseFloat(v)||'');}},
                guarantee:    {caption:'Гарантия', width:75, sort:1, aliace:function(v, cnt, rowData){ return (parseFloat(v)||'');}},
                inBoxes:      {caption:'Кол-во в упаковке / средний вес ', width:95, sort:1, aliace:function(v, cnt, rowData){ return (parseFloat(v)||'');}},
                amountWeight: {caption:'Кол-во (штук) / общий вес', width:95, sort:1, aliace:function(v, cnt, rowData){ return (parseFloat(v)||'');}},
                boxesRols:    {caption:'Кол-во упаковок / кол-во роликов', width:95, sort:1, aliace:function(v, cnt, rowData){ return (parseFloat(v)||'');}},
                description:  {caption:'Примечание', width:200, sort:1, aliace:function(v, cnt, rowData){ return (v||'');}}
            };

            if (claim.perm.products.baseprice == '0') {
                delete(this.grid.head.baseprice);
            }
            if (claim.perm.products.baseprice_in == '0') {
                delete(this.grid.head.baseprice_in);
            }
            if (claim.perm.products.guarantee == '0') {
                delete(this.grid.head.guarantee);
            }

            this.filter.onAfterCreate = function() {
                _self.grid.create();
                _self.grid.fieldControll = 'id';
                _self.grid.lastSelectionData = claim.findAddedMaterial();
                _self.grid.addPostData = _self.subfilter.getValue();
                _self.grid.addPostData.claimId = this.node(claim.formId)['claim[id]'].value;
                if (this.addPostData.clientid) {
                    _self.grid.addPostData.clientid = this.addPostData.clientid
                }
            ...
            this.grid.onDblClick = function(rowIndex, field, node) {
                if (field == 'title' && node.nodeName.toLowerCase() == 'span') {
                    claim.flagInfoIn = false;
                    return;
                }
                claim.showDetailedProductInformationForm(this.data[rowIndex].amountWeight, this.data[rowIndex]['productType'], this.data[rowIndex]['id']);
            };
            ...
            // Установить данные для "Типа товаров"
            filterProductTypeDecorator.modifyProductData(productTypes, this.filter, this.grid);
        ...
        this.overley.onAfterClose = function() {
            delete(claim.overley);
        };
        this.overley.create();
    };
    ...
    this.showDetailedProductInformationForm = function(amountWeight, productType, id)
    {
        if (claim.toFloat(amountWeight) > 0) {
            // Сырьевой товар
            if (productType == 3) {
                raw.getProductAllocation(id);
            }
            // Базовый товар
            else {
                claim.displayEditProductStart(id, 1);
            }
        }
    };
    ...
    this.displayEditProductStart = function(data, ajax)
    {
        if (ajax) {
            ...
            $.ajax({
                url: '/claim/outraw/getinfoblock',
                cache: false,
                data: {
                    itemId: data,
                    claimId: this.node(this.formId)['claim[id]'].value,
                    isRemote: this.blockMode.isRemote()
                },
                type: 'POST',
                dataType: 'json',
                complete: function ()
    ...
    this._displayEditProductFull = function(data)
    {
        ...
        this.overley2.onAfterCreate = function() {
            ...
            if (data.addinfo.length) {
                ...
                var old = {amount:0, boxes:0}, cur = {amount:0, boxes:0};
                for(var i = 0; i < data.addinfo.length; i++) {
                    old.amount += claim.toFloat(data.addinfo[i].lastBlockAmount);
                    old.boxes += claim.toFloat(data.addinfo[i].lastBlockBoxes);
                    cur.amount += claim.toFloat(data.addinfo[i].curBlockAmount);
                    cur.boxes += claim.toFloat(data.addinfo[i].curBlockBoxes);
                }
                ...
                $(row.insertCell(2)).attr({width:100, align:'center'}).html(old.amount);
                $(row.insertCell(3)).attr({width:230, align:'right'}).css({color:'#333'}).html('Кол-во упаковок/кол-во роликов');
                $(row.insertCell(4)).attr({width:100, align:'center'}).html(old.boxes);
            ...
            this.grid2.head = {
                ...
                average: {caption:'Средний вес',        width:100, aliace : function(v, cnt, rowData) { return (claim.toFloat(v)||'');}},
                amount:  {
                    caption:'Кол-во/<br>общий вес<br>(доступно)',
                    width:120,
                    aliace : function(v, cnt, data) {
                        if (data.type == 2) {
                            v = data.maxAmount - data.confirmAmount;
                ...
                boxes:   {caption:'Кол-во упаковок<br>/кол-во роликов<br>(доступно)', width:120, aliace : function(v, cnt, rowData) { return (claim.toFloat(v)||''); }},
                ...
                curBlockAmount : {
                    caption : 'Кол-во/<br>общий вес<br>(по заявке)',
                    width   : 120,
                    aliace  : function(v, cnt, rowData, rowIndex) {
                        var readonly = claim.perm.products.amount < 4 ? 'readonly' : '';
                        var addStyle = (rowData.type == 2 ? 'background-color:#eee; color:#777;' : '');
                        return '<input itype="amount" oninput="claim.overley2.updateCelldata(this, \'curBlockAmount\')" style="text-align:left; width:95%;'
                        + addStyle + '" type="text" value="'
                        + (claim.toFloat(v) ? claim.toFloat(v) : '') + '" '
                        + readonly + ' lang="'
                        + rowIndex + '">';
                    }
                },
                curBlockBoxes : {
                    caption : 'Кол-во упаковок/<br>кол-во роликов<br>(по заявке)',
                    width   : 120,
                    aliace  : function(v, cnt, rowData, rowIndex) {
                        var readonly = claim.perm.products.boxes < 4 ? 'readonly' : '';
                        var addStyle = (rowData.type != 2 ? 'background-color:#eee; color:#777;' : '');
                        return '<input itype="boxes" oninput="claim.overley2.updateCelldata(this, \'curBlockBoxes\')" style="text-align:left; width:95%;'
                        + addStyle + '" type="text" value="'
                        + (claim.toFloat(v) ? claim.toFloat(v) : '') + '" '
                        + readonly + ' lang="'
                        + rowIndex + '">';
            ...
            btok.onclick = function() {
                ...
                if (data.data.length && claim.perm.checkminoutamount == 1) {
                    ...
                    for(var i = 0; i < data.data.length; i++) {
                        allBlockAmount += parseInt(data.data[i].curBlockAmount);
                    }
                ...
                if (data.data.length && data.data[0].type == 1) {
                    ...
                    for(var i = 0; i < data.data.length; i++) {
                        var tmp = data.data[i];
                        if (tmp.amount > tmp.curBlockAmount && tmp.curBlockAmount > 0 && tmp.curBlockAmount%tmp.average) {
                            var text = ' в секции ' + tmp.placing + ' кол-во ';
                        ...
                        // установка значения заблокированного товара по всем секциям
                        allBlockAmount += parseInt(tmp.curBlockAmount);
                    }
                ...
                //Если количестов не соответстует кратности товара запрет
                if (data.data.length && data.data[0].type == 2) {
                    for(var i = 0; i < data.data.length; i++) {
                        if (data.data[i].curBlockBoxes % data.parity) {
        ...
        this.overley2.updateCelldata = function(obj, type) {
            obj.value = obj.value.replace(/[,]/g, '.');
            var data = this.grid2.data[obj.lang];

            if (type == 'curBlockBoxes' && data.type == 2) {
                if (!/^\d+?$/.test(obj.value)) obj.value = Math.ceil(Math.abs(claim.toFloat(obj.value)));
            }else {
                if (data.type == 2) {
                    if (!/^\d+(\.(\d*))?$/.test(obj.value)) obj.value = Math.abs(claim.toFloat(obj.value));
                }else {
                    if (!/^\d+?$/.test(obj.value)) obj.value = Math.ceil(Math.abs(claim.toFloat(obj.value)));
                }
            }
            ...
            if (data.type == 2) {
                if (type == 'curBlockBoxes') {
                    ...
                    data[type] = claim.toFloat(obj.value);
                    ...
                    obj.parentNode.parentNode.previousSibling.firstChild.firstChild.value = amount;
                }else {
                    ...
                    if (amount > 0 && data.curBlockBoxes == 0) {
                        ...
                        obj.parentNode.parentNode.nextSibling.firstChild.firstChild.value = boxes;
                    }else if (amount < claim.precision) {
                        amount = '';
                        data.curBlockBoxes = 0;
                        data.curBlockAmount = 0;
                        obj.parentNode.parentNode.nextSibling.firstChild.firstChild.value = '';
                    }
        ...
        this.overley2.controlButtonsGrid2 = function() {
            var maxBlock = 0; //claim.toFloat((this.tmpdata.type == 2 ? this.tmpdata.boxes : this.tmpdata.amountWeight));
            var lastTotalBlock = 0, curTotalBlock = 0;
            var saveAll = false;
            for(var i = 0; i < this.grid2.data.length; i++) {
                maxBlock += claim.toFloat((this.tmpdata.type == 2 ? this.grid2.data[i].boxes : this.grid2.data[i].amount));
                lastTotalBlock += claim.toFloat(this.grid2.data[i][(this.tmpdata.type == 2 ? 'lastBlockBoxes' : 'lastBlockAmount' )]);
                curTotalBlock  += claim.toFloat(this.grid2.data[i][(this.tmpdata.type == 2 ? 'curBlockBoxes'  : 'curBlockAmount' )]);
                if ((Math.abs(this.grid2.data[i].curBlockAmount - this.grid2.data[i].lastBlockAmount) > claim.precision
                    || Math.abs(this.grid2.data[i].curBlockBoxes - this.grid2.data[i].lastBlockBoxes) > claim.precision)
                ) {
                    saveAll = true;
                }
            }
            ...
            rows = this.grid2.node().firstChild.childNodes.item(1).firstChild.rows || [];
            ...
            for(var i = 0; i < rows.length; i++) {
                ...
                if (clearInp) {
                    clearInp.disabled = !claim.toFloat(this.grid2.data[index].curBlockAmount);
                }
                if ((Math.abs(this.grid2.data[index].curBlockAmount - this.grid2.data[index].lastBlockAmount) > claim.precision
                    || Math.abs(this.grid2.data[index].curBlockBoxes - this.grid2.data[index].lastBlockBoxes) > claim.precision)) {
    ...
    this._displayEditProductSimple = function(data)
    {
        ...
        this.overley2.onAfterCreate = function() {
            ...
            inp.maxValue = data.addinfo.avalibleBlock||0; // data.addinfo.blokMaxInput
            ...
            btok.onclick = function() {
                ...
                tmp = tmp <= claim.overley2.tmpdata.addinfo.avalibleBlock ? tmp : claim.overley2.tmpdata.addinfo.avalibleBlock;
                ...
                if (claim.overley2.tmpdata.type == 1
                    ...
                    && parseFloat(claim.overley2.tmpdata.addinfo.avalibleBlock) > parseFloat(claim.overley2.tmpdata.addinfo.curBlock)
                )  {
    ...
    this._displayEditproductEnd = function(data, ajax)
    {
        if (ajax) {
            ...
            $.ajax({
                url: '/claim/outraw/setblockitem',
                cache: false,
                data: {
                    data: serialize(data),
                    claimId: data.claimId,
                    itemId: data.itemId,
                    isRemote: this.blockMode.isRemote() ? this.blockMode.isRemote() : (claim.isLinkedClaim && claim.isMainProject ? 1 : 0)
                },
                type: 'POST',
                dataType: 'json',
                success: function (data)
                {
                    if (data.error) {
                        ...
                        if (parseInt(claim.perm.products.editproduct) === 4) {

                            claim.overley2.close();

                            claim.showError(data.msg, function ()
                            {
                                claim._displayEditProductFull(this.tmpdata);
                            });
    ...
    this._addClaimProduct = function(data)
    {
        ...
        for(var i = 0; i < head.length; i++) {
            ...
            switch(head[i].lang) {
                ...
                case 'amount':
                    var inp = this.crnode('input');
                    $(inp).attr({type : 'text', value : this.toFloat(String(data.product.amount)), size : 8, readOnly : (this.claimStatus > 0), name:'products[' + number + '][amount]'}).css({'text-align':'left'});
                    cell.appendChild(inp);
                    if (this.claimStatus == 0) {
                        inp.oninput = function(){
                            claim.changeCloseAmountBoxes(this);
                    ...
                    if (claim.isSubsidiaryProject) {
                        ...
                        for (var k = 0; k < data.sections.length; k++) {
                            ...
                            items[data.sections[k].project_depot_id] = {
                                ...
                                title: this.projectsTitlesForButtons[data.sections[k].project_depot_id],
                    ...
                    if (claim.isSubsidiaryProject) {
                        ...
                        $(inp).append(
                            '<div class="dropdown-btn">' +
                                '<input type="image" src="/img/system/delete.png" class="dropdown-toggle_btn" width="16" height="16" lang="' + claim.compresData(langData) + '"/>' +
                            '</div>'
                        );
    ...
    this._updateClaimProduct = function(data)
    {
        ...
        if (this.node(this.formId)['products[' + number + '][amount]']) this.node(this.formId)['products[' + number + '][amount]'].value = data.product.amount;
        ...
        if (this.isSubsidiaryProject) {
            ...
            if (projects.indexOf(parseInt(data.project_depot_id)) === -1) {
                ...
                var item = {
                    ...
                    title: this.projectsTitlesForButtons[data.project_depot_id],
                    product_id: data.itemId
                };
    ...
    this._updateProductSections = function(data)
    {
        ...
        for(var i = 0; i < data.sections.length; i++) {
            ...
            if (data.sections[i].type == 1) {
                ...
                row.insertCell(1).innerHTML = 'Количество : ' + this.toFloat(data.sections[i].curBlockAmount) + this.addBaleOrder(data.sections[i]);
                row.insertCell(2).innerHTML = 'Остаток по секции : ' + data.sections[i].fullamount + "/" + data.sections[i].fullboxes;
                ...
                if (this.perm.products.sectionedit) {
                    ...
                    if (this.perm.products.sectionedit == 1) row.appendChild(inp);
                ...
                if (beforeColspan-1) {
                    row.insertCell(++ind).colSpan = beforeColspan - 1;
                }
                ...
                if (this.perm.products.sectionedit > 1) {
                    ...
                    cell.innerHTML = '<input lang="' + langdatastr + '" oninput="claim.changeSection(this)" name="products['+pnumber+'][sections]['+i+'][amount]" type="text" readonly size=8 value="' + this.toFloat(data.sections[i].curBlockAmount) + '">';
                    cell.align = 'center';
                }
                var cell = row.insertCell(++ind);
                ...
                cell.colSpan = head.length - beforeColspan - 1;
            }
        }
    ...
    {
        if (typeof sectionData.baleOrder !== 'undefined' && sectionData.baleOrder) {
    ...
    this._deleteClaimProduct = function(data, ajax)
    {
        ...
        {
            if (parseInt(claim.claimStatus) === 0) {

                $.ajax({
                    url: '/claim/outraw/delete-baled-product-on-closed-status',
                    cache: false,
                    data: {
                ...
                $.ajax({
                    url: '/claim/outraw/dropproduct',
                    cache: false,
                    data: {
                        itemId : data.itemId,
                        claimId: data.claimId,
                        isRemote: claim.blockMode.isRemote() ? claim.blockMode.isRemote() : (claim.isLinkedClaim && claim.isMainProject ? 1 : 0)
                    },
                    type: 'POST',
                    dataType: 'json',
                    success: function (data)
        ...
        }).then(function (data)
        {
            ...
            if (data.full_delete) {
                ...
                while (rows[i]) {
                    if (parseInt(rows[i].getAttribute('product')) === parseInt(data.productId)) {
                        claim.node('table_products').deleteRow(i); continue;
                    }
    ...
    {
        var totalAmount = 0, totalBoxes = 0, totalSumm = 0;
        var elements = this.node(this.formId).elements;
        for(var i = 0; i < elements.length; i++) {
            switch (true) {
        ...
        if (this.node('autosum-amount')) this.node('autosum-amount').innerHTML = this.round(totalAmount);
    ...
    this.bcControl = function (obj) {
        if (claim.perm.pricebccontrol == 1) {
            var section   = $(obj).parents('#check_show_product').find('.helpButtonForElement');
            section.empty();
    ...
    this.showFile = function (obj) {
        ...
        var vendorLink = '/filemanager/preview/view/type/claim-outraw-document/?claimId=' + id + '&fileName=' + fileName;
        ...
        if (claim.perm.pricebccontrol == 1
            ...
            if (items == 0) {
                ...
                if (textarea.length == 0 && fileCount == 0) {
                    ...
                    itemsId.join(', ');
                    $('tr[data-field-name="outraw/annotation"]').addClass('errorContent');
                    ...
                    $.post('/claim/outraw/sendmailpricelessbaseprice',{
                        claimId   : id,
                        link      : link,
                        manager   : manager,
                        client    : client,
    ...
    this.checkForm = function(form)
    {
        ...
        if (car == 1) {
            if (nextStatusAv > 0 && this.dimensions && !this.dimensions.isValid()) {
                errors.push(' - заполните поле "Габариты"');
            }

            if (this.carProxy && !this.carProxy.isValid()) {
        ...
        if (this.node('check_show_product')) {
            ...
            // В заявке нет товаров и резервов
            if (!productsExists && !claim.hasReserves()) {
        ...
        if (!errors.length && checkSectionAmount && this.managerStatus == 1 && nextStatusAv == 1) {
            $.ajax({
                url      : linkPrefix + '/claim/outraw/check-max-amount',
                cache    : false,
                data     : {data : serialize(products), claimId: this.node(claim.formId)['claim[id]'].value},
                type     : 'POST',
                dataType : 'json',
        ...
        // проверка мнимых тюков
        if (!errors.length ) {
            $.ajax({
                url : linkPrefix + '/claim/outraw/check-imaginary-bales',
                cache    : false,
                data     : {claimId: this.node(claim.formId)['claim[id]'].value},
                type     : 'POST',
                dataType : 'json',
                async    : false,
                success  : function(data) {
        ...
        // Заявка на статусе менеджера + перевод на след статус (закрытие заявки)
        if (!errors.length && parseInt(this.managerStatus) === 1 && parseInt(nextStatusAv) === 1) {
            $.ajax({
                url: '/claim/outraw/check-claim-reserves',
                cache: false,
                type: 'POST',
                dataType: 'json',
                async: false,
                ...
                {
                    claim.showError('<div align=left>' + response.responseText  + '<div>');
                    this.saveForm = false;
                }.bind(this)
            });
        }
    ...
    this.showInfoIn = function(data, ajax)
    {
        ...
        if (ajax) {
            ...
            $(obj).css({'background-image':'url(/img/ld/ld3.gif)', 'background-position':'center center', 'background-repeat':'no-repeat'});
            $.ajax({
                url      : linkPrefix + '/claim/outraw/getinfoin',
                cache    : false,
    ...
    this.sendToCascade = function(params)
    {
        ...
        this.saveForm = true;
        this.node(this.formId).action = linkPrefix + '/claim/outraw/cascade/';
        this.node(this.formId).submit();
    };
    ...
    this.showBlockClaimInfo = function()
    {
        ...
        var html = '<div id="block_info_html" style="position:fixed; top:10px; right:10px; padding:15px; border:1px solid gray; background:rgba(0, 0, 0, 0.5); color:white; border-radius:15px;">'
            ...
            + (claim.isLinkedClaim ? '<input id="check_block_source" class="amd-btn amd-btn_primary" value="Проверить блокировку" type="button">' : '')
            + '</h3>'
    ...
    this.initEditProductDropdownlist = function (element, items)
    {
        element.dropdownList({
            ...
            callback: {
                ...
                {
                    return '<span class="editProduct" data-project_depot_id="' + item.id + '" lang="' + item.product_id + '">' + item.title + '</span>';
                },
                ...
                onItemClick: function (event, dropdownlist)
                {
                    ...
                    // Тип товара
                    var productType = parseInt(tr.find('input.productType').val());
                    var productId   = parseInt(tr.attr('product'));

                    if ($(this).children().first().data('project_depot_id') > 0) {
                        claim.blockMode.setRemote();
                    } else {
                        claim.blockMode.setLocal();
                    }
                    ...
                    // Товар из сырья
                    if (productType === 3) {
                        raw.getProductAllocation(productId);
                    }
                    // Все остальные товары
                    else {
                        claim.displayEditProductStart(productId, true);
                    }
                }
            }
        });
...
window.onload = function () {
    if (typeof(tinyMCE) !== "undefined" && tinymce.get('claim_annotation')) {
        tinyMCE.get('claim_annotation').on('keyup', function(ed,e){
            if ($('tr[data-field-name="outraw/annotation"]').hasClass('errorContent')) {
                $('tr[data-field-name="outraw/annotation"]').removeClass('errorContent');
                $('h2.errorOutContetnMessage').remove();
            }
        });
    }
...
window.onunload = function() {
    if (!claim.saveForm && claim.onunload) {
        $.ajax({
            url      : linkPrefix + '/claim/outraw/dropclaims',
            cache    : false,
    ...
    if (!claim.saveForm && !claim.userBlockInfo) {
        $.ajax({
            url      : linkPrefix + '/claim/outraw/unblockclaim',
            cache    : false,
            data     : {claimId : window.incomeoutUnloadData.cid},
            type     : 'POST',
            dataType : 'html',
