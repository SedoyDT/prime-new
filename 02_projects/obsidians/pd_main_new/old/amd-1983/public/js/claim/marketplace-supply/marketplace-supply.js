...
{
    this.formId           = 'marketplace-supply-form';
    this.precision        = 0.00001;
    this.startNumber      = 0;
    this.timeOut          = null;
    this.сarNumberList    = [];
    this.isEditProductFormDataLoading = false;
    this.disparityChecked = null;
     ...
     * @var String
     */
    this.claimTypeName = 'marketplacesupply';
    ...
    this.blockFormMode = {
        ...
        reset: function () {
            return this.mode === this.types.UNDEFINED;
        }
    };
     ...
     */
    this.getTargetDepotElement = function ()
    {
        return $('select.subsidiary-claim-id')
    };
    ...
    this.getSubsidiaryClaimDepotId = function()
    {
        ...
        var field = this.getTargetDepotElement();
    ...
    this.start = function()
    {
        ...
        $('[name="marketplaceSupply[supplyNumber]"]').mask('9?999999999999999999999');
        ...
        if (window.Dispatcher) {
            new Dispatcher({
                ...
                eventBus: this.eventBus,
                updateCarListActionUrl: '/claim/marketplacesupply/get-dispatcher-drivers/',
            });
        }
    ...
    */
    ...
    this.invoices = {
        ...
        getInvoices : function() {
            ...
            $.ajax({
                url      : '/claim/marketplacesupply/get-invoices',
                cache    : false,
                type     : 'POST',
                data     : data,
                dataType : 'json',
    ...
    this.applyProductFilter = function(filter)
    {
        ...
        $.ajax({
            url      : '/claim/marketplacesupply/getproductfilterdata',
            cache    : false,
            type     : 'POST',
            dataType : 'json',
            data     : data,
    ...
    /**
     * Открытие формы главного склада
     * @returns {undefined}
     */
    this.showMainProjectSelectProducts = function()
    {
        this.blockFormMode.setRemote();
        var maindepotForm = new Claim_Out_Maindepot({});
        maindepotForm.show();
    };
    ...
    this.displayEditProductStart = function(data, ajax)
    {
        if (ajax) {
            ...
            $.ajax({
                url: (claim.blockFormMode.isRemote() && claim.isSubsidiaryProject ? Claim_Out_Maindepot.prototype.getInfoBlockUrl() : '/claim/marketplacesupply/getinfoblock'),
                cache: false,
                data: {
...
';
    ...
    this._displayEditProductSimple = function(data)
    {
        ...
        this.overley2.onAfterCreate = function() {
            ...
            btok.className = 'brownButton bigButton';
            btok.style.width = '125px';
            ...
            btcn.value = 'Отмена';
            btcn.style.width = '125px';
            btcn.className = 'brownButton bigButton';
            btcn.onclick = function() {
                claim.overley2.close();
            };
            cntinfo.appendChild(btcn);
    ...
    this._displayEditproductEnd = function(data, ajax)
    {
        ...
        if (ajax) {
            ...
            $.ajax({
                url      : (claim.blockFormMode.isRemote() || claim.isLinkedClaim && claim.isMainProject ? '/claim/marketplacesupply/set-block-linked-claim' : '/claim/marketplacesupply/setblockitem'),
                cache    : false,
                ...
                complete: function()
                {
                    depotProductsForm.depotListSubFilterToggleOverlay();
                }
            });
            return;
        }
    ...
    this._addClaimProduct = function(data)
    {
        ...
        $(row).append(
            $('<input>').attr({
                type : "hidden",
                value: data.itemId,
                name :'marketplaceSupplyDetails[' + number + '][item_id]'
            })
        );
        ...
        for(var i = 0; i < head.length; i++) {
            ...
            switch(head[i].lang) {
                ...
                case 'inboxes':
                    ...
                    break;
                case 'marketplace_boxes':
                    {
                        let field = $('<input>').attr({
                            name: 'marketplaceSupplyDetails[' + number + '][boxes]',
                            type: "text",
                            value: "",
                            disabled: !(parseInt(this.perm.products.marketplace_boxes) === 4),
                            class: "marketplace-value-field"
                        });

                        $(cell).append(field);
                    }
                    break;
                case 'marketplace_amount':
                    {
                        let field = $('<input>').attr({
                            name: 'marketplaceSupplyDetails[' + number + '][amount]',
                            type: "text",
                            value: "",
                            disabled: !(parseInt(this.perm.products.marketplace_amount) === 4),
                            class: "marketplace-value-field"
                        });
                    ...
                    if (claim.isSubsidiaryProject) {
                        ...
                        for (var k = 0; k < data.sections.length; k++) {
                            items[data.sections[k].project_depot_id] = {
                                ...
                                title: (data.sections[k].project_depot_id == 0 ? window.projectList[claim.currentProjectId] : window.projectList[+data.sections[k].project_depot_id]),
                        ...
                        $(inp).attr({
                            class: 'editProduct claim__input_edit',
                            type: 'button',
                            title: 'редактировать',
                            lang: data.itemId
                        }).css({
                ...
                case 'delproduct':
                    ...
                    cell.appendChild(inp);
                    break;
                case 'marketplace_number':
                    $(cell).html(data.marketplace_number);
                    break;
                case 'product_id':
...
//                    if (this.perm.products.sectionedit == 1)
    ...
    this._deleteClaimProduct = function(data)
    {
        var deletePromise = new Promise(function (resolve, reject) {
            if (claim.claimStatus == 0) {
                ...
                $.ajax({
                    url      : (claim.blockFormMode.isRemote() || claim.isLinkedClaim && claim.isMainProject ? '/claim/marketplacesupply/drop-product-linked-claim' : '/claim/marketplacesupply/dropproduct'),
                    cache    : false,
    ...
    this.sendForm = function(nextStatus)
    {
        ...
        if (claim.perm.pricebccontrol == 1
            ...
            if (items == 0) {
                ...
                var link    = '/claim/marketplacesupply/' + id;
                ...
                if (textarea.length == 0 && fileCount == 0) {
                    ...
                    itemsId.join(', ');
                    $('tr[data-field-name="marketplace-supply/annotation"]').addClass('errorContent');
                    ...
                    $.post('/claim/marketplacesupply/sendmailpricelessbaseprice',{
                        claimId   : id,
                        link      : link,
                        manager   : manager,
                        client    : client,
    ...
    this.isChangeStatusAvailable = function()
    {
        return parseInt(this.node(this.formId)['changeStatusAvaleble'].value) ? 1 : 0;
    }
    ...
    this.showDisparityAgreement = function(disparityList)
    {
        let rows = [];

        $.each(disparityList, (index, disparity) =>
        {
            let productTitle = $(`#table_products_tbody tr[product="${disparity.item_id}"][rowtype="productstart"] [data-field="field1"]`).text();

            rows.push(`
                <tr>
                    <td>${disparity.item_id}</td>
                    <td>${productTitle}</td>
                    <td>${disparity.p_boxes}</td>
                    <td>${disparity.m_boxes}</td>
                    <td>${disparity.p_boxes - disparity.m_boxes}</td>
                    <td>${disparity.p_amount}</td>
                    <td>${disparity.m_amount}</td>
                    <td>${disparity.p_amount - disparity.m_amount}</td>
                </tr>
            `);
        ...
        let template = $(`
            <section>
                <form action="#" method="POST" class="disparity-form">
                    <header>
                        <h1>Расхождение данных</h1>
                    </header>
                    <div class="text-align_center">
                        <div class="disparity-form__message">
                            <div>Внимание! На склад маркетплейс принято меньше отгружаемой с нашего склада продукции.</div>
                            <div>По следующим ID товара должен быть начат спор:</div>
                        </div>
                        <div class="disparity-form__description">
                            <table class="amd-table">
                                <thead>
                                    <tr>
                                        <td>ID товара</td>
                                        <td>Наименование</td>
                                        <td>Упаковок в заявке</td>
                                        <td>Упаковок в поставке</td>
                                        <td>Расхождение</td>
                                        <td>Кол-во в заявке</td>
                                        <td>Кол-во в поставке</td>
                                        <td>Расхождение</td>
                                    </tr>
                                </thead>
        ...
        let disparityAgreementForm = new AjaxForm(
            template,
            {
                width: 900,
                style: 'strict',
                ...
                {
                    form.domObject.find('#agree').click(() =>
                    {
                        this.onDisparityApplied();
                        form.hide();
                    });
                }
            }
        ...
        disparityAgreementForm.show();
    }


    this.validateDisparity = function()
    {
        let products           = this.getProductsFields();
        let marketplaceDetails = this.getMarketplaceSupplyDetailsFields();
        let errors             = [];
        let disparity          = [];

        $.each(products, (productNumber, product) =>
        {
            if (!marketplaceDetails.hasOwnProperty(productNumber)) {
                errors.push(`Не найдено детализации для товара ${product.product_id} обратитесь к администратору`);
            } else {
                let marketplaceRow = marketplaceDetails[productNumber];

                if (parseInt(marketplaceRow.item_id) !== parseInt(product.product_id)) {
                    errors.push(`Не найдено детализации для товара ${product.product_id} обратитесь к администратору`);
                } else if (marketplaceRow.amount - product.amount > 0) {
                    errors.push(`Для товара ${product.product_id} кол-во ${marketplaceRow.amount} в поставке больше кол-ва ${product.amount} в заявке`);
                } else if (marketplaceRow.boxes  - product.boxes > 0) {
                    errors.push(`Для товара ${product.product_id} кол-во упаковок ${marketplaceRow.boxes} в поставке больше кол-ва упаковок ${product.boxes} в заявке`);
                }
                else if (
                    (
                        (Math.abs(marketplaceRow.amount - product.amount) > 0.00001) ||
                        (Math.abs(marketplaceRow.boxes  - product.boxes)  > 0.00001)
                    )
                ) {
                    disparity.push({
                        item_id : product.product_id,
                        p_amount: product.amount,
                        p_boxes : product.boxes,
                        m_amount: marketplaceRow.amount,
                        m_boxes : marketplaceRow.boxes
                    });
                }
            }
        });
        ...
        if (errors.length) {
            alert(errors.join("\n"));
        } else if (disparity.length) {
            this.showDisparityAgreement(disparity);
        } else {
            this.onDisparityApplied();
        }
    }
    ...
    this.onDisparityApplied = function()
    {
        this.disparityChecked = true;
        $('[data-action-claim-send="1"]').click();
    }


    this.checkForm = function(form)
    {
        // Проверять только на статусе менеджера и если ранее проверка не проводилась
        if (this.isChangeStatusAvailable() && this.managerStatus && this.disparityChecked !== true) {
            // Метод повторно вызовет checkForm
            this.validateDisparity();
            return false;
        }

        this.disparityChecked = false;
            ...
            if (!els[i].name.indexOf('products') && this.node('check_show_product')) {
                ...
                switch (els[i].name) {
                    ...
                    case 'claim[carProxy]':
                        ...
                        break;
                    case 'marketplaceSupply[supplyNumber]':
                    {
                        if (// На статусе менеджер
                            parseInt(this.claimStatus) === 101 &&
                            // Есть право редактировать
                            parseInt(this.perm.supply_number) === 4 &&
                            // Введено неккоретное значение
                            !parseInt(els[i].value)
                        ) {
                            errors.push(' - поле № поставки в маркетплейс указано неверно');
                        }
                    }
                        break;
                }
        ...
        this.validateMarketplaceSupplyDetails(errors, nextStatusAv);
        ...
        if (!errors.length && checkSectionAmount && this.claimStatus != 0) {
            $.ajax({
                url      : '/claim/marketplacesupply/checksectionamount',
                cache    : false,
        ...
        if (!errors.length && checkSectionAmount && this.managerStatus == 1 && nextStatusAv == 1) {
            $.ajax({
                url      : '/claim/marketplacesupply/check-max-amount',
                cache    : false,
                data     : {data : serialize(products), claimId: this.node(claim.formId)['claim[id]'].value},
                type     : 'POST',
                dataType : 'json',
        ...
        if (!errors.length && parseInt(nextStatusAv) === 1 && claimIdField.length && linkedProductionField.length && parseInt(linkedProductionField.val())) {
            $.ajax({
                url: '/claim/marketplacesupply/validate-linked-production-products',
                cache: false,
                data: {
     ...
     * @returns {{}}
     */
    this.validateMarketplaceSupplyDetails = function (errors, changeStatus)
    {
        if (parseInt(changeStatus) === 1 &&
            // Заявка у менеджера
            parseInt(this.claimStatus) === 101 &&
            // Есть право на редактирование данных о поставке
            (
                parseInt(this.perm.products.marketplace_amount) === 4 ||
                parseInt(this.perm.products.marketplace_boxes,) === 4
            )
        ) {
            let marketplaceSupplyDetailsFields = this.getMarketplaceSupplyDetailsFields();

            $.each(marketplaceSupplyDetailsFields, (index, field) =>
            {
                let boxes  = parseInt(field.boxes);
                let amount = parseInt(field.amount);

                if (isNaN(boxes) || boxes < 0) {
                    errors.push(`Для товара ID ${field.item_id} не заполнено значение "Маркетплейс Кол-во уп-к / кол-во роликов"`);
                }

                if (isNaN(amount) || amount < 0) {
                    errors.push(`Для товара ID ${field.item_id} не заполнено значение "Маркетплейс Кол-во / общий вес"`);
                }
            });
        }
    }
     ...
     * @returns {{}}
     */
    this.getMarketplaceSupplyDetailsFields = function ()
    {
        let storage = {};
        this.parseFieldsData(storage, $('[name^="marketplaceSupplyDetails"]'));

        return storage.hasOwnProperty('marketplaceSupplyDetails') ? storage.marketplaceSupplyDetails : {};
    }
     ...
     * @returns {{}}
     */
    this.getProductsFields = function ()
    {
        let storage = {};
        this.parseFieldsData(storage, $('[name^="products"]'));

        return storage.hasOwnProperty('products') ? storage.products : {};
    }
     ...
     * @param fields
     */
    this.parseFieldsData = function(storage, fields)
    {
        $.each(fields, (index, field) =>
        {
            let name = $(field).attr('name');
            let path = name.replace(/\]/g, '').replace(/\[/g, '.').split('.');
            ...
            while (path.length) {
                ...
                if (typeof currentStorage[pathPart] == 'undefined') {
                    currentStorage[pathPart] = {};
                }

                if (!path.length) {
                    currentStorage[pathPart] = field.value;
                }

                currentStorage = currentStorage[pathPart];
            }
        });
    }
    ...
    this.showInfoIn = function(data, ajax)
    {
        ...
        if (ajax) {
            ...
            $(obj).css({'background-image':'url(/img/ld/ld3.gif)', 'background-position':'center center', 'background-repeat':'no-repeat'});
            $.ajax({
                url      : '/claim/marketplacesupply/getinfoin',
                cache    : false,
    ...
    this.sendToCascade = function(params)
    {
        ...
        this.saveForm = true;
        this.node(this.formId).action = '/claim/marketplacesupply/cascade/';
    ...
    this.showBlockClaimInfo = function()
    {
        ...
        + (claim.showLinkedProjectBlockCheck ? '<input id="check_block_source" class="brownButton bigButton" value="Проверить блокировку" type="button">' : '')
        + '</h3>'
...
window.onload = function () {
    if (typeof(tinyMCE) !== "undefined" && tinymce.get('claim_annotation')) {
        tinyMCE.get('claim_annotation').on('keyup', function(ed,e){
            if ($('tr[data-field-name="marketplace-supply/annotation"]').hasClass('errorContent')) {
                $('tr[data-field-name="marketplace-supply/annotation"]').removeClass('errorContent');
                $('h2.errorOutContetnMessage').remove();
            }
        });
    }
...
window.onunload = function() {
    if (!claim.saveForm && claim.onunload) {
        $.ajax({
            url      : '/claim/marketplacesupply/dropclaims',
            cache    : false,
    ...
    if (!claim.saveForm && !claim.userBlockInfo) {
        $.ajax({
            url      : '/claim/marketplacesupply/unblockclaim',
            cache    : false,
            data     : {claimId : window.incomeoutUnloadData.cid},
            type     : 'POST',
            dataType : 'html',
